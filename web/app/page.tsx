  ...
  return (
    <div>
      ...
      {providers.length > 0 && (
        <div className="card mb-6">
          <div className="mb-3 flex items-center justify-between">
            <h3 className="text-sm font-semibold text-fg">Provider API keys</h3>
              ...
              <span
                ...
                className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[11px] font-semibold ${
                  configured
                    ...
                    : "border-line/10 bg-ink-900/40 text-fg-muted"
                }`}
              >
                <span
      ...
      <div className="mb-6 grid grid-cols-2 gap-4 sm:grid-cols-4">
        <StatCard label="Runs stored" value={loaded ? runs.length : "–"} />
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <div className="card">
          ...
          {runs.length === 0 ? (
            <p className="text-sm text-fg-muted">No runs yet — start one in the Playground.</p>
          ) : (
            <div className="flex flex-col gap-2">
              {[...runs].reverse().slice(0, 6).map((r) => (
                <Link
                  key={r.run_id}
                  href="/runs"
                  className="flex items-center justify-between rounded-lg border border-line/5 bg-ink-900/40 px-3 py-2 text-xs hover:border-brand-500/30"
                >
                  <span className="font-mono text-fg-muted">{r.run_id}</span>
                  <span className="text-fg">{r.kind}</span>
                  <span className="text-fg-muted">{r.strategy}</span>
                </Link>
              ))}
            </div>
          )}
        ...
        <div className="card">
          <h3 className="mb-3 text-sm font-semibold text-fg">Strategies available</h3>
          <div className="flex flex-wrap gap-2">
            {(opts?.strategies || []).map((s: any) => (
              <Chip key={s.name}>{s.name}</Chip>
            ))}
          </div>
          <h3 className="mb-3 mt-5 text-sm font-semibold text-fg">Metric presets</h3>
          <div className="flex flex-wrap gap-2">
            {(opts?.metrics_presets || []).map((m: string) => (
              <Chip key={m}>{m}</Chip>
            ))}
          </div>
          <h3 className="mb-3 mt-5 text-sm font-semibold text-fg">Bundled &amp; benchmark datasets</h3>
          <div className="flex flex-wrap gap-2">

...
| `router.py` | Unified `completion()` / `embedding()` / `rerank()` addressed as `provider/model`. Routes to the OpenAI SDK (with a base-url override) for OpenAI-compatible providers, and to native SDKs for Anthropic/Cohere/Voyage/Bedrock/Azure/HuggingFace-local. Auto-loads a project `.env`. |

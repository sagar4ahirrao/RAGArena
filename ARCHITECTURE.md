...
| `index.py` | `VectorIndex` (zero-config chunk + embed + search, with a swappable `backend=`), `TextChunker` (recursive character splitter with overlap), `MultimodalDocument` (typed text/table/image/equation chunks). |
| `backends.py` | `VectorBackend` interface + 9 implementations: `numpy` (default, exact cosine, no deps), `faiss`, `chroma`, `qdrant`, `lancedb`, `weaviate`, `elasticsearch`, `redis`, `pgvector`; `register_backend()` for anything else. Backends receive already-embedded vectors, so swapping one changes only storage/ANN search — making vector DBs themselves comparable on identical embeddings (all 9 verified to return identical scores against live servers). Defaults favour *exact* search over ANN, since an evaluation must measure the retrieval under test rather than an approximation of it — notably Elasticsearch, whose own default quantizes vectors to int8 and can reorder close results. |

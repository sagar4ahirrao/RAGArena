...
| `backends.py` | `VectorBackend` interface + `numpy` (default, exact cosine, no deps), `faiss`, `chroma`, `qdrant`, `lancedb` implementations; `register_backend()` for server-backed stores. Backends receive already-embedded vectors, so swapping one changes only storage/ANN search — making vector DBs themselves comparable on identical embeddings. |
